  ...
  async launch(message, query) {
    ...
    message.channel.send({embed: listEmbed}).then((msg) => {
      ...
      collector.on('collect', async (msgCollected) => {
        ...
        const info = await ytdl.getBasicInfo(`https://www.youtube.com/watch?v=${items.id.videoId}`);
        const song = {
          songID: items.id.videoId,
          guildID: message.guild.id,
          title: items.snippet.title,
          thumbnail: items.snippet.thumbnails.high.url,
          time: JSON.parse(JSON.stringify(info)).length_seconds*1000,
          service: 'YouTube',
